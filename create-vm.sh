...
VM_IP_PREFIX="${VM_IP_PREFIX:-192.168.1.11/24}"
...
function geniso() {
	...
	[[ "$PUB_NET" = yes ]] && {
	    [[ $(virsh domiflist $VM_NAME | grep virtio | grep PUB | wc -l) = 0 ]] && {
	...
	[[ "$INT_NET" = yes ]] && {
	    [[ $(virsh domiflist $VM_NAME | grep virtio | grep INT | wc -l) = 0 ]] && {
	...
	sed -i -r -e "s,__VM_IP_prefix__,$VM_IP_PREFIX,g" ./network-config
...
[[ "$1" = "--geniso-only" ]] && {
	echo "Generating new data source ISO"
        geniso
	exit 0
}
...
lvremove /dev/$LVM_VG/$VM_NAME --yes
...
virsh pool-refresh $VIRSH_POOL

virsh vol-dumpxml --pool $VIRSH_POOL ${VM_NAME}
LVM_VOL_PATH=$(virsh vol-list --pool $VIRSH_POOL | grep -P "\b${VM_NAME}\b" | awk '{print $2}')
